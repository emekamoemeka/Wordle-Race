    ...
    if (((/[a-zA-Z]/).test(event.key) || event.key == 'Enter' || event.key == 'Backspace') && gameOver == false) {
        ...
        if (event.key == "Enter") {
            for (let i = 0; i < nameIndex; i++){
                nameEntry[i].style.backgroundColor = "#414242"
                nameEntry[i].style.border = "2px solid #414242"
                nameEntry[i].style.color = "white"
            ...
            for (let i = 0; i < 5; i++) {
                if (leaderBoardScores[i].innerHTML < score) {
                    ...
                    leaderBoardNames[i].style.color = 'white'
